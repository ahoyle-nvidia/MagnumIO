...
int main(int argc, char **argv)
{
    ...
    std::cout << "registering device memory of size :" << allocationSizeA << std::endl;
    ...
    std::cout << "registering device memory of size :" << allocationSizeB << std::endl;
    // registers device memory
    ...
    checkCudaErrors(simpleMallocMultiDeviceMmapResize(d_A, va_size, &allocationSizeA, size, backingDevices, mappingDevices));
    ...
    ret = cuFileWrite(cf_handle_A, (void*)d_A, allocationSizeA, 0, 0);
    if (ret < 0) {
            ...
            std::cout << "written bytes to d_A:" << ret << std::endl;
            ret = 0;
    }
    ...
    ret = cuFileRead(cf_handle_A, (void*)d_A, allocationSizeA, 0, 0);
    if (ret < 0) {
            if (IS_CUFILE_ERR(ret))
