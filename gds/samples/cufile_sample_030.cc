 ...
 *
 * Sample cuFileBatchIOSubmit Read/Write Test for various combination of files opened in regular, O_DIRECT, unaligned I/O, unregistered buffers, registered buffers, GPU memory and system memory.
...
typedef enum {
        MEM_TYPE_CUDA_MALLOC = 0,
        MEM_TYPE_CUDA_MALLOC_HOST,
        MEM_TYPE_MALLOC,
        MEM_TYPE_MMAP,
        MAX_MEM_TYPES
}memTypes;
...
int submit_batch(int argc, char *argv[], bool write) {
	...
	unsigned num_completed = 0;
        bool registerBuf = 0;
        uint8_t to_compare[MAX_BUFFER_SIZE];
        uint8_t fill_pattern[MAX_BUFFER_SIZE];

        memset(fill_pattern, 0xab, MAX_BUFFER_SIZE);

	if(argc < 6) {
                std::cerr << argv[0] << " <filepath> <gpuid> <num batch entries> <Buf Register 0 - register all buffers, 1 - unregistered buffers> <nondirectflag> "<< std::endl;
                exit(1);
        }
	...
	for(i = 0; i < batch_size; i++) {
		devPtr[i] = NULL;
                if(i % MAX_MEM_TYPES == MEM_TYPE_CUDA_MALLOC) {
		        check_cudaruntimecall(cudaMalloc(&devPtr[i], size));
                } else if(i % MAX_MEM_TYPES == MEM_TYPE_CUDA_MALLOC_HOST) {
		        check_cudaruntimecall(cudaMallocHost(&devPtr[i], size));
                } else if(i % MAX_MEM_TYPES == MEM_TYPE_MALLOC) {
                        devPtr[i] = malloc(size);
                } else if(i % MAX_MEM_TYPES == MEM_TYPE_MMAP) {
                        devPtr[i] = mmap(NULL, size,
                                        PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0 );
                }
                if(i % MAX_MEM_TYPES < MEM_TYPE_MALLOC) {
                        if(write) {
                                check_cudaruntimecall(cudaMemset((void*)(devPtr[i]), 0xab, size));
                                check_cudaruntimecall(cudaStreamSynchronize(0));	
                        } else {
                                check_cudaruntimecall(cudaMemset((void*)(devPtr[i]), 0, size));
                                check_cudaruntimecall(cudaStreamSynchronize(0));	
                        }

                } else {
                        if(write)
                                memset((void*)(devPtr[i]), 0xab, size);
                        else
                                memset((void*)(devPtr[i]), 0, size);
                }
	}
	// filler
	...
	// registers device memory
        if(registerBuf) {
                std::cout << "registering memory of size :" << size << std::endl;
                for(i = 0; i < batch_size; i++) {
	...
	std::cout << "Batch IO Get status done got completetions for " << nr << " events" << std::endl;
        ...
        if(!write) {
                bool status = 0;
                memset(to_compare, 0, MAX_BUFFER_SIZE);
                for(i = 0; i < batch_size; i++) {
                        if(i % MAX_MEM_TYPES < MEM_TYPE_MALLOC) {
                                check_cudaruntimecall(cudaMemcpy(to_compare, devPtr[i],
                                                        MAX_BUFFER_SIZE, cudaMemcpyDeviceToHost));
                        } else {
                                memcpy(to_compare, devPtr[i], MAX_BUFFER_SIZE);
                        }
                        status = memcmp(to_compare, fill_pattern, MAX_BUFFER_SIZE);
                        assert(status == 0);
                }
        }
out4:
...
out2:
	for(i = 0; i < batch_size; i++) {
                if(i % MAX_MEM_TYPES == MEM_TYPE_CUDA_MALLOC)
                        ...
                        check_cudaruntimecall(cudaFreeHost(devPtr[i]));
                else if(i % MAX_MEM_TYPES == MEM_TYPE_MALLOC)
                        free(devPtr[i]);
                else if(i % MAX_MEM_TYPES == MEM_TYPE_MMAP)
                        munmap(devPtr[i], size);
	}
out1:
	// close file
	for(i = 0; i < batch_size; i++) {
...
int main(int argc, char *argv[]) {
        submit_batch(argc, argv, 1);
        submit_batch(argc, argv, 0);
        return 0;
}
